...
p_testdata=PostProcessor(outputDir=".",
	...
	modules=[DijetSkimmer(year=Year.k2016, source=Source.kDATA, dataset=Dataset.kJetHT)],
	provenance=True)
p_testdata.run()
...
p_testmc=PostProcessor(outputDir=".",
	...
	modules=[DijetSkimmer(year=Year.k2017, source=Source.kMC, dataset=Dataset.kNone)],
	provenance=True)
p_testmc.run()
